...
| **T656** — resilient ONNX embedding-model acquisition (`TurOnnxModelDownloader`: follow HuggingFace Git-LFS redirects, retry/backoff, persistent cache dir, integrity check) | Internal reliability — makes the existing local/HuggingFace ONNX embedding warm-up download robust; no new capability or admin/API/SDK surface. The only knobs are ops-level (`turing.genai.embedding.local.cache-dir`, `…download.*`), covered by the [embedding-models](./embedding-models.md) provider docs where relevant |
