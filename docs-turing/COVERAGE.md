...
| **Block AZ** (T740–T744, T747, T748) — Governed LLM Gateway: OpenAI-compatible `/v1/*` inbound API; virtual keys (`sk-turing-…`) with per-key allowed-models scope, monthly budget + auto-downgrade, hard cap, rate limit; `x-turing-cache` / `x-turing-guardrails` / `x-turing-rag-site` / `x-turing-tools` headers; `turing-agent:` / `turing-sn:` / `turing-local:` model-name routing; per-key spend dashboard + `/bento/gateway` admin | [Governed LLM Gateway](./llm-gateway.md) | ✅ |
