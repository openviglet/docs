...
| **Key Token** | n/a | Bearer token for authentication |
| **Basic Username / Password** | n/a | Basic auth credentials |
...
| Configuration | Default | Description |
|---|---|---|
| **JDBC URL** | n/a | PostgreSQL connection string (e.g., `jdbc:postgresql://localhost:5432/turing`) |
| **Username** | n/a | Database user |
...
| **Dimensions** | n/a | Vector dimensionality: must match the [embedding model](./embedding-models.md) |
| **Distance Type** | n/a | `COSINE_DISTANCE`, `EUCLIDEAN_DISTANCE`, or `INNER_PRODUCT` |
| **Index Type** | n/a | `HNSW` or `IVFFLAT` |
...
| **Token** | n/a | Authentication token (`username:password` format) |
| **Embedding Dimension** | n/a | Vector dimensionality: must match the embedding model |
| **Metric Type** | n/a | `COSINE`, `L2`, or `IP` (inner product) |
| **Index Type** | n/a | `HNSW`, `IVF_FLAT`, `IVF_SQ8`, or `DISKANN` |
| **Index Parameters** | n/a | JSON string with index-specific params (e.g., `{"M":16,"efConstruction":200}`) |
...
| **Quantization** | n/a | n/a | IVF_SQ8 | INT8 / INT4 / 7-bit / binary |
...
| **Managed cloud** | n/a | Any managed PostgreSQL | Zilliz Cloud | n/a |
