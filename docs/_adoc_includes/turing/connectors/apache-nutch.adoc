...
You can change the Semantic Navigation website in the following ways:
...
2. Change using `turing.site`, via `nutch-site.xml` or as a command line parameter. If using `turing.force=true` as parameter. This setting will override `solr.server.url`.
