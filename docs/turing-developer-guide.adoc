...
*Installation and Configuration Hours*: 2 hours

Turing will be installed only using OpenNLP and H2 bank embedded in Turing AI itself.
...
Ideal for development and testing environment. Because it requires fewer components and little memory usage

==== Docker Compose

*Installation and Configuration Hours*: 16 hours

Turing and its dependencies will be raised using Docker Compose, which will include the following services:

* MariaDB – to store Turing system tables
* Solr – Used by Turing's Semantic Navigation and Chatbot
* Nginx – WebServer for Turing to use port 80
* Turing AI.
...
===== Target Audience
Ideal for environment that needs better complexity but avoids installing product by product. It can be used in an QA or Production environment.

==== Kubernetes

*Installation and Configuration Hours*: 24 hours

Turing and its dependencies will be raised using Kubernetes, which will include the following services:
...
Ideal for customers who want to use cloud solutions like Google, AWS, Oracle, etc. Can be used in Production and scalable.
...
* MariaDB – to store Turing system tables
* Solr – Used by Turing's Semantic Navigation and Chatbot
* Apache – WebServer for Turing to use port 80
* Turing AI.
...
Ideal for customers who prefer the on-premise structure. And you want to have the services installed directly on the servers. It can be used in Development, QA and Production.

=== Connectors

Turing has several connectors to allow you to index the contents in Semantic Navigation:
...
On average it will take *16 hours* to configure the connector and have the first indexing version in Turing.