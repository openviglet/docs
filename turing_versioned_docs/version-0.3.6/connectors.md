...
Extract to /appl/viglet/turing/wem:
```shell
...
|-----------|----------|-------------|
| `--all` / `-a` | No | Index all instances |
| `--content-type` / `-c` | No | Content type XML name |
| `--host` / `-h` | Yes | Content Management server host |
| `--siteName` / `-s` | Yes | WEM site name |
| `--username` / `-u` | Yes | Username |
| `--password` / `-p` | No | Password |
| `--working-dir` / `-w` | Yes | Working directory |
