...
2. Edit the `cda.classpath` file of Management and Delivery Stages and add:
```properties
...
|-----------|----------|---------|-------------|
| `--all` / `-a` | No | false | Index all instances of all content types |
| `--content-type` / `-c` | No | - | XML name of the content type to index |
| `--guids` / `-g` | No | - | Path to file containing GUIDs |
| `--host` / `-h` | Yes | - | Content Management server host |
| `--page-size` / `-z` | No | 500 | Page size for processing |
| `--password` / `-p` | No | - | User password |
| `--siteName` / `-s` | Yes | Sample | WEM site name |
| `--username` / `-u` | Yes | - | Username for login |
| `--working-dir` / `-w` | Yes | - | Working directory with vgncfg.properties |
